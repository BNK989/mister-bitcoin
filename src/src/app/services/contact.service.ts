...
export class ContactService {
    ...
    public loadContacts() {
        return from(storageService.query<Contact>(ENTITY)).pipe(
            tap((contacts) => {
    ...
    public getContactById(id: string): Observable<Contact> {
        return from(storageService.get<Contact>(ENTITY, id)).pipe(
            catchError((err) => throwError(() => `Contact id ${id} not found! ${err}`)),
        )
    }
    ...
    private _updateContact(contact: Contact) {
        return from(storageService.put<Contact>(ENTITY, contact)).pipe(
            tap((updatedContact) => {
                const contacts = this._contacts$.value
                this._contacts$.next(
                    contacts.map((contact) => (contact._id === updatedContact._id ? updatedContact : contact)),
                )
            }),
            retry(1),
            catchError(this._handleError),
